...
# Définition du modèle (copiez les classes VisionTransformer et TransformerEncoderLayer ici)
...
model.load_state_dict(torch.load('./Data/vit_cifar10.pth'))
model.eval()
...
plt.xlabel('Patchs')
plt.ylabel('Patchs')
plt.savefig('./Data/attention_map.png')
plt.close()
...
def show_image(img, title):
    ...
    plt.title(title)
    plt.axis('off')
    plt.savefig('./Data/original_image.png')
    plt.close()

show_image(image[0], f'Image de test - Label : {label}')

print("Les graphiques ont été enregistrés sous './Data/attention_map.png' et './Data/original_image.png'.")
